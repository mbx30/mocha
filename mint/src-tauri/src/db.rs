...
impl Database {
    ...
    pub fn update_invoice(
        ...
        let conn = self
            ...
            .map_err(|_| rusqlite::Error::InvalidQuery)?;
        conn.execute(
            "UPDATE invoices SET status = ?1, subtotal = ?2, tax_rate = ?3, tax_amount = ?4, total = ?5,
             internal_notes = ?6, customer_notes = ?7, client_id = ?8, issue_date = ?9, due_date = ?10,
             payment_terms = ?11, updated_at = datetime('now') WHERE id = ?12",
            params![
                status,
                subtotal,
                tax_rate,
...
mod tests {
    ...
    #[test]
    fn convert_estimate_to_invoice_links_records() {
        use tempfile::tempdir;

        let dir = tempdir().expect("tempdir");
        let db = Database::new(dir.path().to_path_buf()).expect("db");
        let client = db
            .create_client("Test", "Test Co", "test@example.com", "", "", "")
            .expect("client");
        let est = db
            .create_estimate("EST-001", "2026-12-31")
            .expect("estimate");
        db.add_estimate_line_item(est.id, "Cards", "materials", 500.0, 0.25)
            .expect("line");
        db.update_estimate(
            est.id,
            "approved",
        ...
        let inv_data = db.convert_estimate_to_invoice(est.id).expect("convert");
        assert_eq!(inv_data.invoice.subtotal, 125.0);
        assert_eq!(inv_data.invoice.source_estimate_id, Some(est.id));
        assert_eq!(inv_data.line_items.len(), 1);

        let est_data = db.get_estimate_data(est.id).expect("est");
        assert_eq!(est_data.estimate.converted_invoice_id, Some(inv_data.invoice.id));
    }
}

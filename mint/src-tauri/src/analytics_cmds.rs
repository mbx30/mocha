...
/// error categories.
#[tauri::command]
pub fn get_analytics_dashboard(db: State<'_, Database>) -> Result<AnalyticsDashboard, String> {
    let summary = db.get_analytics_summary().map_err(|e| e.to_string())?;
    let client_pass_rates = db.get_client_pass_rates().unwrap_or_default();
    let average_turnaround_hours = db.get_average_turnaround_hours().unwrap_or(0.0);
    let common_error_categories = db.get_common_error_categories().unwrap_or_default();

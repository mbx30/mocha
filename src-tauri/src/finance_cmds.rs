...
pub fn export_invoices_csv(
    ...
    db.export_invoices_csv(&path, invoice_ids.as_deref())?;
    Ok(output_path)
}
...
pub fn send_invoice_email(
    ...
    let subject = format!("Invoice {} from Mint", inv.invoice_number);
    let mut body = format!(
        "Invoice: {}\nIssue date: {}\nDue date: {}\nTotal: {:.2} {}\n\n",
    ...
    crate::email::send_email_via_smtp(&settings, &to, &subject, &body, None)
}
